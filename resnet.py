...
def main():
    ...
    n_classes = 2
    model_path = "resnet50_model.pth"  # 모델 저장 경로
    ...
    train_dataset = ImageFolder(root='dataset_final/train', transform=transform)
    val_dataset = ImageFolder(root='dataset_final/validation', transform=transform)
    test_dataset = ImageFolder(root='dataset_final/test', transform=transform)
    ...
    for epoch in range(epochs):
        ...
        if epochs_no_improve >= patience:
            print("Validation loss가 개선되지 않아 조기 종료합니다.")
            break
